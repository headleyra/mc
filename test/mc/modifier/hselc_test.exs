defmodule Mc.Modifier.HselcTest do
  ...
  describe "Mc.Modifier.Hselc.modify/2" do
    test "uses `args` as a CSS selector targeting HTML content in the `buffer`", do: assert true
    ...
    test "targets embedded elements", %{html: html} do
      assert Hselc.modify(html, "table") ==
        {:ok, "Lorem ipsum The quick fox\nBook A great read!"}
    end
    ...
    test "targets lists of elements", %{html: html} do
      assert Hselc.modify(html, ".desc, .deets") ==
        {:ok, "Foo bar\nA great read!\nJohn Doe"}
    end
