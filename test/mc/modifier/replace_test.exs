defmodule Mc.Modifier.ReplaceTest do
  ...
  describe "m/3" do
    test "splits `args` into 'search' and 'replace' terms and returns `buffer` with replacements applied" do
      ...
      assert Replace.m("one two", "one four 4", %{}) == {:ok, "four 4 two"}
      assert Replace.m("2 x", "x is company", %{}) == {:ok, "2 is company"}
    ...
    test "works with a URI-encoded 'replace' term" do
    ...
    test "errors when the 'replace' term is missing" do
      assert Replace.m("3s a crowd", "bad", %{}) == {:error, Mc.Modifier.Replace, :bad_search_replace, "bad", []}
    end

    test "errors when the 'search' and 'replace' terms are missing" do
      assert Replace.m("bish bosh", "", %{}) == {:error, Mc.Modifier.Replace, :bad_search_replace, "", []}
    end

    test "errors when the 'search' regex is bad" do
      assert Replace.m("stuff", ")", %{}) == {:error, Mc.Modifier.Replace, :bad_search_replace, ")", []}
      assert Replace.m("(foo)", "*", %{}) == {:error, Mc.Modifier.Replace, :bad_search_replace, "*", []}
    end
    ...
    test "allows error-tuples to pass through" do
      assert Replace.m({:error, Mod, :fuel, "low", []}, "", %{}) == {:error, Mod, :fuel, "low", []}
    end
  end
end
