defmodule Mc.Modifier.HtabTest do
  ...
  describe "m/3" do
    ...
    test "errors unless there are exactly 2 selectors", %{html: html} do
      assert Htab.m(html, "", %{}) == {:error, "Mc.Modifier.Htab: CSS selector parse error"}
      assert Htab.m(html, "p", %{}) == {:error, "Mc.Modifier.Htab: CSS selector parse error"}
      assert Htab.m(html, "table tr td", %{}) == {:error, "Mc.Modifier.Htab: CSS selector parse error"}
    end
    ...
    test "allows error tuples to pass through" do
      assert Htab.m({:error, "some reason"}, "", %{}) == {:error, "some reason"}
    end
  end
end
