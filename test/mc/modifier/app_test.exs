defmodule Mc.Modifier.AppTest do
  ...
  describe "modify/3" do
    test "builds an 'app script' given an 'app key' and runs it against the `buffer`" do
      ...
      assert App.modify("stuff", "", %Mc.Mappings{}) == {:ok, "stuff"}
      assert App.modify("", "", %Mc.Mappings{}) == {:ok, ""}
    end

    test "replaces placeholders (::1 => arg1, ::2 => arg2, ...) before running the script" do
      assert App.modify("a b c", "app3 arg1 arg2", %Mc.Mappings{}) == {:ok, "arg1 arg2 c"}
      assert App.modify("a:a", "app5 alpha", %Mc.Mappings{}) == {:ok, "alpha:alpha"}
      assert App.modify("", "app7 un deux trois", %Mc.Mappings{}) == {:ok, "1 => un, 2 => deux, all => un deux trois"}
    ...
    test "replaces the 'all args' placeholder (::: => arg*)" do
      assert App.modify("", "app7 yab dab do", %Mc.Mappings{}) == {:ok, "1 => yab, 2 => dab, all => yab dab do"}
    end
